package pl.leszczenko.streetnoise.utils;
...
import pl.leszczenko.streetnoise.fragments.FragmentFiles;

import pl.leszczenko.streetnoise.fragments.ServiceLauncherFragmentPlay;
import pl.leszczenko.streetnoise.fragments.ServiceLauncherFragmentRecord;
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id, IDrawerItem drawerItem) {
                ...
                if (drawerItem != null) {
                    ...
                    if (drawerItem.getIdentifier() == 1) {
                        activity.getSupportFragmentManager().beginTransaction().replace(R.id.frame_container, new ServiceLauncherFragmentPlay()).commit();
                    } else if (drawerItem.getIdentifier() == 2) {
                        activity.getSupportFragmentManager().beginTransaction().replace(R.id.frame_container, new ServiceLauncherFragmentRecord()).commit();
                    } else if (drawerItem.getIdentifier() == 3) {
                        activity.getSupportFragmentManager().beginTransaction().replace(R.id.frame_container, new FragmentFiles()).commit();
    ...
    public static Drawer.Result createCommonDrawer(final ActionBarActivity activity, Toolbar toolbar, AccountHeader.Result headerResult) {
        ...
        Drawer.Result drawerResult = new Drawer()
                ...
                .addDrawerItems(
                        ...
                        new PrimaryDrawerItem().withName(R.string.info_fr2).withIcon(GoogleMaterial.Icon.gmd_audiotrack).withIdentifier(2),
                        // Вывод НЕ для Google Play
                        new PrimaryDrawerItem().withName(R.string.info_fr3).withIcon(GoogleMaterial.Icon.gmd_list).withIdentifier(3)
